...
public class GeminiColorVariantService implements ColorVariantImageService {
    ...
    private String buildEnhancedOriginalPrompt(String productName, byte[] originalImageBytes) {
        return String.format(
                "Enhance the quality of this image while keeping the same product and person.");
    }
    ...
    private String buildColorVariantPrompt(String productName, String color, byte[] originalImageBytes) {
        return String.format(
                "Create an identical copy of this image with ONLY the %s color changed to %s. " +
                        ...
                        "- Same exact pose and body position " +
                        "- Same background (every detail, shadow, texture) " +
                        "- Same lighting (direction, intensity, shadows) " +
                        "- Same camera angle and composition " +
                        "- Same %s fit, wrinkles, and fabric texture " +
                        "- Same image quality and sharpness " +
        ...
        try (CloseableHttpClient httpClient = HttpClients.createDefault()) {
            ...
            return httpClient.execute(request, response -> {
                ...
                if (statusCode == 200) {
                    ...
                    HttpEntity entity = response.getEntity();
                    String errorBody = entity != null ? new String(entity.getContent().readAllBytes())
                            : "No error body";
        ...
        try {
            ...
            for (String color : colorVariants) {
                if (!existingUrls.containsKey(color)) {
                    String variantFileName = String.format("fallback-variant-%s-%s.jpg",
                            color.toLowerCase(),
                            UUID.randomUUID().toString().substring(0, 8));
