...
public class ProductService {
	...
	public ProductResponse createProduct(CreateProductRequest request) {
		...
		try {
			...
			Map<String, String> variantImageUrls = colorVariantImageService.generateColorVariantImages(
				originalImageData, productType, uniqueColors);
	...
	public ProductResponse createProductWithAIVariants(MultipartFile image, String name, String description, 
			Integer basePrice, UUID brandId, UUID categoryId, String genderStr, String variantsJson, String metadataJson) {
	...
	private ProductResponse createProductWithGeneratedImages(CreateProductRequest request, Map<String, String> variantImageUrls) {
		...
		if (mainImageUrl != null) {
			ProductImage mainImage = ProductImage.builder()
					...
					.altText("Enhanced product image")
					.isPrimary(true)
					.sortOrder(0)
					.build();
		...
		for (ProductVariant variant : savedVariants) {
			...
			if (variantImageUrl != null) {
				ProductImage variantImage = ProductImage.builder()
						...
						.altText("Variant image for " + variantColor)
						.isPrimary(true)
						.sortOrder(0)
						.build();
	...
	private String extractProductTypeFromName(String productName) {
		...
		if (lowerName.contains("shirt")) return "shirt";
		if (lowerName.contains("polo")) return "polo";
		if (lowerName.contains("jacket")) return "jacket";
		if (lowerName.contains("sweater") || lowerName.contains("pullover")) return "sweater";
		if (lowerName.contains("pants") || lowerName.contains("trousers")) return "pants";
		if (lowerName.contains("jeans")) return "jeans";
		if (lowerName.contains("dress")) return "dress";
		if (lowerName.contains("hoodie")) return "hoodie";
